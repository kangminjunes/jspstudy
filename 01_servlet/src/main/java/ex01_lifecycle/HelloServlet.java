 ...
 * 3. javax.servlet.http.HttpServlet 클래스 상속 받는다.
 * 4. jsp/servlet container(톰캣)에 저장되기 떄문에 동일한 이름의 서블릿을 2개 이상 만들 수 없다.
 *  톰캣이 없으면 실행이 되지않는다. 즉 동일한 환경상태야 된다는 의미이다.
 ...
 * 
 * 1. 형식
 *  protocol://host:prt/contextPath/URLMapping
 * 2. contextPath
 *  1) 프로젝트가 실행될 떄 가지는 기본 경로를 의미한다. (Context Root)
 *  2) 수정 방법
...
public class HelloServlet extends HttpServlet {
	...
	/**
	 * 3.service() 
	 *   1) 클라이언트가 요청할 떄 자동으로 호출되는 메소드이다.
	 ...
	 *   3)  POST 방식으로 요청하는 방법
   *     (1) <form method="pst" action="/servlet/HelloServlet">
   *     (2) $.ajax({
   *           type: 'post',
   *           url: '/servlet/HelloServlet', 
   *           ...
