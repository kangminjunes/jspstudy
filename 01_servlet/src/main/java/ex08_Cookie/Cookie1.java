...
public class Cookie1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  // 즉 모든 쿠키 값은 String 타입으로 저장한다. 쿠키에 저장되는데이터는 전부 ""를 써야한다
	  Cookie cookie2 = new Cookie("age","20");
	  // 공백은 유효하지 않는 문자이므로 인코딩이 필요하다.
	  ...
	  cookie2.setPath("/servlet/cookie1"); // 서블릿경로   : request.getRequestURI()
	                                       // cookie3은 경로 설정을 생략햇으므로 컨텍스트패스에 저장된다.
	  
	  // 쿠키가 유지되는 시간 설정하기 (생략하면 세션쿠키가 된다. 브라우저를 닫으면 지워진다.)
    // 쿠키 삭제하는법은 해당 유지시간을 0으로 설정후 재저장하면 삭제된다.
	  cookie1.setMaxAge(60 * 60);          // 1시간 60초 x 60 =3600초
	  cookie2.setMaxAge(60 * 60 * 24 * 7); // 7일   60초 x 60 1시간 * 24 1일 * 7일된다.
	                                       // cookie3은 시간 설정을 생략했으므로 세션쿠기가 생성된다.
