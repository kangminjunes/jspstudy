...
/**
 * Servlet implementation class ResponseServiet
 */
@WebServlet("/response")
public class ResponseServiet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ResponseServiet() {
        super();
    ...
    String region = request.getParameter("region");
	  ...
	  /*
	   *  응답 (Resonse)
	   *  1. 서버가 클라이언트에게 데이터를 보내는 것을 의미한다. 클라이언트 <- 서버
	   *  2. HttmpServletResopse 클래스가 응답을 처리한다.
	  ...
	  /*
	  해당같이 원래 2번에 나눠 적지 않고 한번에 작성 가능하다.
	  response.setContentType("text/html"); 
	  response.setCharacterEncoding("UTF-8");
	  */ 
    response.setContentType("text/html; charset=UTF-8"); // 다음과 같이 한번에 작성가능
