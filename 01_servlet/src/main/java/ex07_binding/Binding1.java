...
public class Binding1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	   ...
	   *  binding
	   *  
	   *  1. 값을 저장할 수 있는 영역에 속성(Attribute)의 형태로 값을 저장하는 것을 의히만다.
	   ...
	   *     2) getAttribute("속성")     : Object 타입의 값을 반환한다. 캐스팅(형변환 타입)이 필요할 수 있다.
	  ...
	  // HttppServletRequest 영역에 값을 저장하기 (이미 매개변수에 선언되어 있다.)
	  request.setAttribute("msg", "HttpServletRequest");
	  
	  // Binding2 서블릿으로 이동(2가지 방식이 존재한다. forward,redirect 로 가능하다.)
	  // (HttpServletRequest의 전달이 가능한 forward를 이용)
