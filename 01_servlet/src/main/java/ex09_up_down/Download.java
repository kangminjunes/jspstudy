...
public class Download extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	  ...
	  // 해당 리눅스에서 실행시 역슬래쉬(\)를 슬래쉬로(/) 변경해야한다.
	  File file = new File(path);  // new File("C:\storage\404.jpg")
	  
	  // 다운로드 할 파일을 읽어 들일 바이트 기반 입력 스트림 생성
	  BufferedInputStream in = new BufferedInputStream(new FileInputStream(file));
	  
	  // 다운로드용 응답을 만들려면 응답 헤더 "Content-Disposition"에 "attachment" 값을 저장해야 한다.
	  response.setHeader("Content-Disposition", "attachment");    // 다운로드할 떄 파일 저장 대화상자가 나타난다.
	                                                                                               // 리눅스에서 사용시 \ 역슬래쉬 말고 /슬래쉬 사용해야함  
	  response.setHeader("content-Disposition", "attachment; filename=" + path.substring(path.lastIndexOf("\\") + 1)); // 다운로드 할 떄 작성한 filename으로 곧바로 저장된다.
