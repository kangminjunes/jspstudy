...
public class Upload extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	   ...
	   * 2. COS File Upload Libraray - cos-22.05.zip 다운로드
	   ...
	   * cos.jar
	   * 1. 업로드 전용 라이브러리이다.
	   * 2. 파일 첨부 폼 양식은 enctype="multipart/form-dat"로 설정하는데 이렇게 되면 HttpServletRequest를 사용할수 없다.
	   * 3. HttpServletRequest 대신 사용할 MultipartRequest 클래스를 지원하는 라이브러리이다. 
	   */
	  
	  // 첨부 파일의 경로 
	  // 서버에 업로드하는 경우 리눅스 어딘가에 업로드하는거니 주소를 바꾸면된다.
    ...
    String lastModified = new SimpleDateFormat("yyyy-MM-dd").format(attach.lastModified());  // 최종 수정일
	  // 1024 kb = 1MB , 1025 kb = 2MB 취급되기에 계산식이 필요하다.
