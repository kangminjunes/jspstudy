...
public class BoardDao {
  ...
  // Connection Pool 관리 DataSouce 객체 선언
  private DataSource dataSource;
  
  // singleton Pattern으로 BoardDao 객체 생성
  private static BoardDao dao = new BoardDao();
  private BoardDao() {
    // META-INF/context.xml에 있는 <Resouce name="jdbc/oraclexe"/> 태그 내용을 읽어서 DataSource 객체 생성하기
    try {
  ...
  public int register(BoardDto dto) {
    ...
    try {
      ...
      ps.setString(1, dto.getTitle());   // 1번쨰 물음표(?)에 dto.getTitle() 전달하기
      ps.setString(2, dto.getContent()); // 2번쨰 물음표(?)에 dto.getContent() 전달하기
